...
- Model registry defined in `karukan-engine/models.toml`; default models use Q5_K_M quantization. Config `model`/`light_model` also accept `hf:owner/repo/filename.gguf` and local `.gguf` paths (`Backend::from_spec`), with `tokenizer.json` taken from the same repo / same directory
