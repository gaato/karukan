...
`persona` によく書く話題や自分のことを表す自由なテキスト（例: `プログラミング`）を設定すると、モデルへ渡す左コンテキストの先頭に `{persona}。{文脈}` の形でそのまま毎回付与され、それに合わせた変換が出やすくなります（例: 英語表記を優先したければ `programming` のような英単語を入れる）。ラベルや構造は付けません — このサイズのモデルは擬似的な構造化プロンプトを理解しないため、キーワードが直前の文章として見えるだけの単純な仕組みです。ライブ変換・Space変換の両方に適用されます。適用中は aux のモード表示にペルソナの中身（モデルが受け取る実効値）がそのまま表示されます（例: `⚡[あ]P:プログラミング`）。末尾25文字まで使用されるため、10〜20文字程度を推奨します。空（既定）で無効です。（設定名が `profile` でないのは、プロフィール切替機能の名前として予約しているためです。）
