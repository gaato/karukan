...
- Conversion persona: `[conversion] persona` in config.toml (free text about the user or their topics, e.g. 「プログラミング」) is prepended to every model lctx as a plain `{persona}。{ctx}` prefix inside `run_kana_kanji_conversion` (`persona_lctx`, last 25 chars, empty = off — no labels: the model is too small for pseudo-structured prompts, the keywords just read as the preceding sentence), so live chunks and Space conversion both carry it and it participates in the conversion cache key; while active the aux mode indicator shows the effective persona content (e.g. ⚡[あ]P:プログラミング). The config key is `persona` because "profile" is reserved for planned per-user profile switching (separate model/learning cache/user dicts/persona per profile, e.g. private vs work)
