...
- Conversion profile: `[conversion] profile` in config.toml (free text about the user, e.g. 「田中太郎/エンジニア」) is prepended to every model lctx as `プロフィール:{p}・発言:{ctx}` inside `run_kana_kanji_conversion` (`profile_lctx`, last 25 chars, empty = off), so live chunks and Space conversion both carry it and it participates in the conversion cache key
